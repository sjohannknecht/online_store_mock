...
jest.mock("../pages/Products/Products", () => () => (
  <>
    <h1>Products</h1>
    <p>{"You're on the Products page"}</p>
  </>
));
jest.mock("../pages/Product/Product", () => () => (
  <>
    <h1>Product</h1>
    <p>{"You're on the Product page"}</p>
  </>
));
jest.mock("../pages/ShoppingCart/ShoppingCart", () => () => (
  <>
    <h1>Cart</h1>
    <p>{"You're on the Shopping Cart page"}</p>
  </>
));
...
jest.mock("../pages/DataPrivacy/DataPrivacy", () => () => (
  <>
    <h1>Data Privacy</h1>
    <p>{"You're on the Data Privacy page"}</p>
  </>
));
jest.mock("../pages/Imprint/Imprint", () => () => (
  <>
    <h1>Imprint</h1>
...
describe("Router", () => {
  ...
  test("renders the products page when starting from the route /products", () => {
    ...
    render(<RouterProvider router={router}></RouterProvider>);
    expect(
      screen.getByText(/you're on the products page/i),
    ).toBeInTheDocument();
  });
  ...
  describe("renders the corresponding page when clicking a Link to a route:", () => {
    test("from Link to /products to Products page", async () => {
      ...
      expect(
        await screen.findByText(/you're on the products page/i),
      ).toBeInTheDocument();
    });
    ...
    test("from Link to /shopping-cart to ShoppingCart page", async () => {
      ...
      expect(
        await screen.findByText(/you're on the shopping cart page/i),
      ).toBeInTheDocument();
    });
    ...
    test("from Link to /user to User page", async () => {
      ...
      expect(
        await screen.findByText(/you're on the user page/i),
      ).toBeInTheDocument();
    });
    ...
    test("from Link to /data-privacy to DataPrivacy", async () => {
      ...
      expect(
        await screen.findByText(/you're on the data privacy page/i),
      ).toBeInTheDocument();
    });
    ...
    test("from Link to /imprint to Imprint", async () => {
      ...
      expect(
        await screen.findByText(/you're on the imprint page/i),
      ).toBeInTheDocument();
    });
  });
});
